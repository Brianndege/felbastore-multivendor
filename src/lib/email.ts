...
export async function sendVerificationEmail(email: string, token: string, userType: "user" | "vendor") {
  const verificationUrl = `${process.env.NEXTAUTH_URL}/auth/verify-email?token=${token}&type=${userType}`;
...
export async function sendPasswordResetEmail(email: string, token: string, userType: "user" | "vendor") {
  const resetUrl = `${process.env.NEXTAUTH_URL}/auth/reset-password?token=${token}&type=${userType}`;
