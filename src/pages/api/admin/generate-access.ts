...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const limiterKey = `admin-generate-access:${hashIdentifier(ipAddress)}`;
  ...
  if (!isAllowedAdminGenerator(email)) {
    ...
    return res.status(403).json({ error: "Only the configured admin email can generate access keys" });
  }
  ...
  return res.status(200).json({ loginUrl, expiresAt: expiresAt.toISOString() });
}
